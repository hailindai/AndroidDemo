...
import com.dreamguard.aframe.utils.APKVersionCodeUtil;
...
public class DiskCacheManager {
    ...
    public DiskCacheManager(String uniqueName) {
        try {
            //先关闭已有的缓存
            ...
            File cacheFile = getCacheFile(AppContextUtil.getInstance(), uniqueName);
            mDiskLruCache = DiskLruCache.open(cacheFile, APKVersionCodeUtil.getVersionCode(AppContextUtil.getInstance()), 1, 1024 * 1024 * 10);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
